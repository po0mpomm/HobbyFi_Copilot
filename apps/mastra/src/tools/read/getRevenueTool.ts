...
export const getRevenueTool = createTool({
  ...
  inputSchema: z.object({
    vendor_id: z.string().describe('Vendor UUID — injected by the gateway; do not ask the user for this'),
  ...
  execute: async ({ context }) => {
    ...
    const payouts = await prisma.payouts.findMany({ where: { vendor_id, ...(venue_id ? { venue_id } : {}) }, select: { gross_amount: true, commission_deducted: true, net_amount: true, status: true } });
